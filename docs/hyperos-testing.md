...
1. 安装 Debug APK。
2. 确认设备已安装并启动 Shizuku。
3. 启动 App，按提示授予音频和电话相关权限。
4. 在 `Shizuku` 区域确认状态为已连接，点击 `请求 Shizuku 授权` 并允许 HyperRing。
5. 选择一个较短的音频文件。
6. 选择 `SIM 1`，点击 `导入并应用`。
7. 用另一部手机拨打 SIM 1，确认铃声是否生效。
8. 回到 App，选择 `SIM 2`，再次点击 `导入并应用`。
9. 拨打 SIM 2，确认铃声是否生效。
10. 再测试 `SIM 1 + SIM 2`，确认双卡是否都被设置。

## 当 SIM 单独设置失败时

如果 SIM1/SIM2 设置后没有实际生效，或者 HyperOS 铃声设置页仍显示 `More`，请使用 `诊断` 区域辅助校准。
...
HyperRing 会在小米、Redmi、POCO 设备上通过 Shizuku 默认尝试这组 key。下面的手动校准流程用于处理不同 HyperOS 版本或不同机型不兼容的情况。
...
HyperRing 默认写入已实测的 HyperOS 双卡 key。不同机型如果 key 不一致，可以在真机校准后手动填写 SIM1/SIM2 主 key。
